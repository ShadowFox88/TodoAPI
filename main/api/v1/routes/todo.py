...
from main.core.schema.todo import TodoBase, Todo, TodoCreate, TodoRead
...
async def create_task(task: TodoBase, logged_in_details: Annotated[Users, Depends(get_logged_in_details)], session: AsyncSession = Depends(get_session)):
    todo = Todo(**task.dict())
...
async def delete_task(todo_id: UUID, logged_in_details: Annotated[Users, Depends(get_logged_in_details)], session: AsyncSession = Depends(get_session)):
...
async def get_task(todo_id: UUID, logged_in_details: Annotated[Users, Depends(get_logged_in_details)], session: AsyncSession = Depends(get_session)):
    todo = await session.get(Todo, todo_id)
    if not todo:
        raise HTTPException(status_code=404, detail="Task not found")
    if todo.owner_id != logged_in_details["User"].id:
